<template>
  <div class="white--text d-flex justify-center">
    <div class="col-7 mt-5">
      <h2>New Content Provider Application</h2>
      Lorem ipsum dolor sit, amet consectetur adipisicing elit. Enim dolor ea
      necessitatibus. Id voluptatum laborum quod non omnis debitis, quasi sed
      obcaecati beatae vel aliquam labore. Perferendis, numquam nisi? Quaerat.
      ...
      <v-form class="mt-5" ref="form" v-model="valid">
        ...
        <h4 class="mb-2">Tell us about the content you will be uploading</h4>
        <v-select
          :items="mediaTypes"
          dark
          required
          v-model="application.media"
          label="What media types will you be uploading"
          outlined
        ></v-select>
        <v-select
          v-model="application.content"
          label="What content types will you be uploading"
          :items="contentTypes"
          attach
          chips
          required
    ...
    <v-dialog v-model="termsModal" dark width="600px">
      <v-card style="background: #03293d;">
        <v-card-title>
          <span class="headline">Staxave Terms of Service</span>
        </v-card-title>
        <v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="submitApplication">
...
export default {
  ...
  methods: {
    ...mapActions("user", ["apply"]),
    ...mapActions("creds", ["callWithToken"]),
    submitApplication: async function() {
      try {
        this.loading = true;
        let response = await this.callWithToken({
          params: {
        ...
        this.$toast.info(response.message, { timeout: 5000 });
        this.loading = false;
      } catch (error) {
